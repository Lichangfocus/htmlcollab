...
```bash
npx htmlcollab-cli login --email <邮箱> --name <用户名>   # 首次使用，免验证
...
**登录注意**：`login` 不带参数是交互式提问，agent 环境会挂起——先检查 `~/.htmlcollab.json` 是否已有 apiToken；没有就**先向用户询问邮箱和用户名**，再带 `--email --name` 非交互执行。
