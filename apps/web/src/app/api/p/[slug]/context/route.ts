...
export async function GET(req: Request, ctx: { params: Promise<{ slug: string }> }) {
  ...
  const version = await db
    .prepare('SELECT number, html FROM versions WHERE page_id = ? ORDER BY number DESC LIMIT 1')
    .bind(page.id)
    .first<{ number: number; html: string }>()
  ...
  const md = buildContext(page, version, results, url.origin)
